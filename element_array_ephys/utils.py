...
def make_ephys_recording(table, key, table_map):
    sess_dir = find_full_path(get_ephys_root_data_dir(), get_session_directory(key))
    inserted_probe_serial_number = (table_map[""] * probe.Probe & key).fetch1("probe")
    ...
    if acq_software == "SpikeGLX":
        ...
        root_dir = find_root_directory(
            get_ephys_root_data_dir(), probe_data.recording_info["recording_files"][0]
        )
        table.EphysFile.insert(
            [
...
make_lookup = {
    "EphysRecording": make_ephys_recording,
}
